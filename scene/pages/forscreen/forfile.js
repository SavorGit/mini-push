...
var file_max_size = app.globalData.file_max_size;
...
Page({
  ...
  getFileDetail:function(e){
    ...
    utils.PostRequest(api_v_url + '/file/detail', {
      ...
      var share_files = data.result.forscreen_file;
      var share_file_num = data.result.forscreen_file_num;
  ...
  uploadFile:function(e){
    var that = this;
    var file_exts = app.globalData.config_info.file_exts
    wx.chooseMessageFile({
      ...
      success(res) {
        ...
        if(file_max_size<file_size){
          var show_max_file_size = file_max_size / (1024 * 1024)
          app.showToast('投屏文件不可以超过' + show_max_file_size + 'M',);
        }else {
  ...
  uploadOssFile:function(policy, signature, file_path,  file_name, file_size, ){
    ...
    var upload_task = wx.uploadFile({
      ...
      success: function (res) {
        ...
        that.setData({addDisabled:false,upDisabled:false})
      }
    });
